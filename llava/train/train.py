...
class ModelArguments:
    ...
    vision_tower: str = field(default=None)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # size 336 for CLIP, 224 for Meta
        size = 336
        if isinstance(i, int):
            sources = [sources]
        ...
        if 'image' in sources[0]:
            ...
            import numpy as np
            vid_sf, vid_clip = self.list_data_dict[i]['video']

            #load feature
            sf_feature = np.load(vid_sf)
            clip_feature = np.load(vid_clip)
            # cat and normalize
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.vision_tower_name = model_args.vision_tower_name
